...
//! let tournament = Selector::tournament(3);
//!
//! // Roulette wheel selection
//! let roulette = Selector::roulette();
//! ```
//!
...
//! # Population Window
//!
//! Selection always operates on the full *effective* population provided by the
//! optimization service:
//! - **Generational schedules** pass the entire previous generation.
//! - **Rolling schedules** pass the most recent `population_size` completed
//!   evaluations.
//! - Other schedulers can define their own window as long as it contains at least
//!   `2 * tournament_size` evaluated genotypes.
//!
//! This keeps tournament and roulette selection simple—no extra sampling knobs—
//! while guaranteeing that selection pressure reflects the actual live population.
...
/// let tournament_selector = Selector::tournament(3);
///
/// // Roulette wheel selection (fitness-proportionate)
/// let roulette_selector = Selector::roulette();
///
/// // High selection pressure tournament
/// let elite_selector = Selector::tournament(7);
/// ```
...
pub enum SelectionError {
    ...
    ///
    /// Tournament selection requires at least `2 * tournament_size` evaluated candidates because
...
impl Selector {
    ...
    /// let large_sample = Selector::roulette();
    /// ```
    ///
